package com.Veiled.Utils.CustomSlideView;
...
public class SliderView extends View {
    ...
    public void setIntervalHours( int i_startHour, int i_endHour){
        startHour = i_startHour;
        endHour = i_endHour;
    }
	...
	protected void onDraw(Canvas canvas) {
		...
		if(!initialisedSlider) {
            ...
            int oneHour = (int)(13 * (sliderWidth - sliderWidth/15 ))/(15 * 24) ;
            int newEnd  = 24 - endHour;

			pointKnobStart.x = (int)((sliderWidth - sliderWidth/15)/15) + (startHour * oneHour - 5 * startHour/2);    //(int)((sliderWidth - sliderWidth/12)/6);
			pointKnobStart.y = (int)(sliderHeight/2.0);
			pointKnobEnd.x = (int)(13 * (sliderWidth - sliderWidth/15)/15) -  (newEnd * oneHour - 5 * newEnd/2);                                          //(int)(7*(sliderWidth - sliderWidth/12)/12);
			pointKnobEnd.y = (int)(sliderHeight/2.0);
		    
			knobs[0] = new Knob(getContext(),R.drawable.patrat, pointKnobStart);
			knobs[1] = new Knob(getContext(),R.drawable.patrat, pointKnobEnd);
			knobs[0].setID(1);
			knobs[1].setID(2);
			setStartKnobValue(startHour);
			setEndKnobValue(endHour);
			knobValuesChanged(true, true, getStartKnobValue(), getEndKnobValue());
		
			paintSelected = new Paint();//the paint between knobs
			paintSelected.setColor(0xFFE15E4A);
			paintNotSelected = new Paint();//the paint outside knobs
			paintNotSelected.setColor(0xFF494949);
		...
		//the slider data
		for(int i=0;i<=6;i++) {
             paintText.setTextSize((float)sliderHeight/10);
             canvas.drawText(i*4+":00", (float)((i+1) * (sliderWidth - sliderWidth/10)/7 - 90), (float)(sliderHeight/3), paintText);
			//canvas.drawLine((float)((i+1)*(sliderWidth - sliderWidth/14)/14), (float)(sliderHeight/2.8),
			//				(float)((i+1)*(sliderWidth - sliderWidth/14)/14), (float)(sliderHeight/2.3), paintText);
		}
